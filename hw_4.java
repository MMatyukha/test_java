...
public class h_w4 {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            g.playerTurn();
            g.printMap();
            if (g.checkWin(g.player)) { System.out.println("Поздравляю! Вы  выиграли"); break; }
            if (g.isMapFull()) { System.out.println("Игра  окончена. Ничья"); break; }

            //AI-1 turn
            g.aiTurn(g.cpu1);
            g.printMap();
            if (g.checkWin(g.cpu1)) { System.out.println("Поздравляю! Выиграл  компьютер"); break; }
            if (g.isMapFull()) { System.out.println("Игра  окончена. Ничья"); break; }
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < size; j++) {
                ...
                if ((countV == size - 1) && (countVNull == 1)) {
                    //System.out.println("Комп  всегда  прав! vert line = " + (i + 1)); // i - horiz line
                ...
                if ((countH == size - 1) && (countHNull == 1)) {
                    //System.out.println("Комп  всегда  прав! horiz line = " + (i + 1)); // i - horiz line
                    for (int k = 0; k < size; k++) {
                        if (map[i][k] == cNull) {
                            map[i][k] = c;
                            return;
            ...
            if ((countDiagonalA == size - 1) && (countDANull == 1)) {
                //System.out.println("Комп  всегда  прав! diagA line \\");
                for (int j = 0; j < size; j++) {
                    if (map[j][j] == cNull) {
                        map[j][j] = c;
                        return;
